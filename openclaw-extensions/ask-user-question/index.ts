 ...
 * and waits for the user's response via an HTTP callback to NukemAI.
 *
 * This enables delete-confirmation modals on the NukemAI desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for NukemAI desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Use a factory so the tool is only available for NukemAI local-session candidates.
    // IM channel sessions (qqbot, dingtalk, weixin, feishu, etc.) get null → tool hidden.
    api.registerTool((ctx) => {
      // Enable for NukemAI desktop sessions across agents and delegated child sessions.
