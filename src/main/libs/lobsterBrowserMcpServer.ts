...
const toolDefinitions = [
  ...
  ['${BrowserCredentialLoginTool.Name}', {
    ...
    reason: { type: 'string' },
  }, 'Sign in through an isolated NukemAI login view with a credential saved for the current website. The password is never returned to the Agent. This may ask the user for approval; after approval, continue the task without asking the user to type or paste the password.'],
];
const tools = toolDefinitions
  .filter(([name]) => !credentialOnly || name === '${BrowserCredentialLoginTool.Name}')
  .map(([name, properties, description]) => ({
  name,
  description: description || 'Operate the NukemAI in-app browser.',
...
async function callBridge(name, args) {
  if (!bridgeUrl || !bridgeSecret) {
    ...
    return errorResult('NukemAI browser bridge is not configured.');
  }
  let response;
  try {
  ...
  if (!response.ok) {
    const message = payload && typeof payload.error === 'string'
      ? payload.error
      : 'NukemAI browser bridge returned HTTP ' + response.status + '.';
    writeDiagnostic(
...
async function callTool(name, args) {
  ...
  if (!imageBase64 || !filePath) {
    return errorResult('NukemAI browser screenshot data or destination path is missing.');
  }
...
async function handleRequest(message) {
  ...
  if (message.method === 'initialize') {
    ...
    if (typeof name !== 'string' || !tools.some((tool) => tool.name === name)) {
      result = errorResult('Unknown NukemAI browser tool.');
    } else {
...
const prepareLobsterBrowserMcpRuntime = (
  ...
  if (!options.electronNodeRuntimePath.trim()) {
    throw new Error('NukemAI browser MCP requires an Electron Node runtime path.');
  }
  if (!options.bridgeUrl.trim() || !options.bridgeSecret) {
    throw new Error('NukemAI browser MCP requires an active browser bridge.');
  }
