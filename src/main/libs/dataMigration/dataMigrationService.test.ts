...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  expect(entries).toContain(`${APP_NAME}/.lobsterai-migration.json`);
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, APP_NAME, '.lobsterai-migration.json'), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(`does not contain ${APP_NAME} user data`);
});
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, APP_NAME, '.lobsterai-migration.json');
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, APP_NAME, 'backups', 'sqlite', 'snapshots', 'lobsterai-latest.sqlite'), 'legacy-source-backup');
  writeFile(path.join(extractRoot, APP_NAME, 'sqlite-backups', 'lobsterai-latest.sqlite'), 'legacy-source-legacy-backup');
