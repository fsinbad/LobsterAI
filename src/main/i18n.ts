 ...
 *   setLanguage('en');
 *   const label = t('trayShowWindow'); // "Open NukemAI"
...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    trayCompletedTaskTooltip: 'NukemAI - {count} 个任务已完成',
    traySettings: '设置',
    trayQuit: '退出',

    // Quit confirmation (native dialog shown on user-initiated quit)
    appQuitConfirmTitle: '退出 NukemAI？',
    appQuitConfirmDetail: 'NukemAI 关闭期间，定时任务不会运行，也无法回复 IM 消息。',
    ...
    browserCredentialApprovalSubtitle: 'NukemAI 将在隔离页面中填写，密码不会提供给 Agent。',
    ...
    browserCredentialSelectionSubtitle: '选定后，NukemAI 将自动完成账号密码填写。',
    ...
    coworkErrorLobsterAILoginExpired: '登录状态已过期，请重新登录后继续使用 NukemAI 套餐模型。',
    ...
    coworkGatewaySessionSyncTimeout: 'OpenClaw 引擎响应缓慢，消息尚未发送。请等待 1~2 分钟后重新发送；若频繁出现，请检查系统内存与磁盘占用，并将 NukemAI 加入杀毒软件白名单。',
    ...
    dataMigrationBackupDialogTitle: '备份 NukemAI 数据',
    dataMigrationRestoreDialogTitle: '导入 NukemAI 数据备份',
    dataMigrationBackupArchiveFilter: 'NukemAI 备份包',
    dataMigrationAllFilesFilter: '所有文件',
    dataMigrationBackupBlockedByActiveWorkloads:
      '当前有正在运行的 Agent 或定时任务，请停止或等待任务完成后再备份。',
    dataMigrationRestoreProgressTitle: '正在导入 NukemAI 数据',
  ...
  en: {
    ...
    trayCompletedTaskTooltip: 'NukemAI - {count} completed task(s)',
    traySettings: 'Settings',
    trayQuit: 'Quit',

    // Quit confirmation (native dialog shown on user-initiated quit)
    appQuitConfirmTitle: 'Quit NukemAI?',
    appQuitConfirmDetail: 'While NukemAI is closed, scheduled tasks will not run and IM messages will not be answered.',
    ...
    browserCredentialApprovalSubtitle: 'NukemAI fills the isolated page without revealing the password to the Agent.',
    ...
    browserCredentialSelectionSubtitle: 'NukemAI will fill the selected account automatically.',
    ...
    coworkErrorLobsterAILoginExpired:
      'Your login session has expired. Sign in again to continue using NukemAI plan models.',
    ...
    coworkGatewaySessionSyncTimeout: 'The OpenClaw engine is responding slowly and your message has not been sent. Please wait a minute or two and resend. If this happens frequently, check system memory and disk usage, and add NukemAI to your antivirus allowlist.',
    ...
    // Data migration dialogs
    dataMigrationBackupDialogTitle: 'Back Up NukemAI Data',
    dataMigrationRestoreDialogTitle: 'Import NukemAI Data Backup',
    dataMigrationBackupArchiveFilter: 'NukemAI Backup',
    ...
    dataMigrationRestoreProgressTitle: 'Importing NukemAI data',
    dataMigrationRestoreProgressDesc:
      'Restoring the backup and validating data. NukemAI will restart automatically when finished.',
