...
1. **Configure accounts in NukemAI Settings > Email.**

NukemAI writes `accounts.json` for multi-account setups. Existing `.env` files are still read as a legacy single-account fallback.
...
- `accounts.json` - Multi-account credentials managed by NukemAI Settings
